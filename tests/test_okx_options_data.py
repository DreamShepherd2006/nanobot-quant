...
def _inst(strike: int, typ: str, exp: int, family="BTC-USD"):
    return {
        "instId": f"{family}-{exp}-{strike}-{typ}",
        "instFamily": family, "state": "live", "optType": typ,
        "ctType": "inverse", "stk": str(strike), "expTime": str(exp),
        "ctVal": "1", "ctMult": "0.01", "uly": family,
    }


def _inst_set(family="BTC-USD"):
    insts = []
...
def _tickers_dict():
    ...
    for exp in (_D1, _D2, _D3):
        for typ in ("C", "P"):
            iid = f"BTC-USD-{exp}-80000-{typ}"
            out[iid] = {"instId": iid,
                        "bidPx": "0.05" if typ == "C" else "0.0045",
                        "askPx": "0.051" if typ == "C" else "0.0046"}
    return out
...
class _FakePublic:
    def get_instruments(self, instType="OPTION", instFamily="BTC-USD", **kw):
        return {"code": "0", "data": _inst_set(instFamily), "msg": ""}

    def get_opt_summary(self, instFamily="BTC-USD", **kw):
...
def test_fetch_chain_selected_expiry_and_full_range(fake_sdk):
    chain = od.fetch_chain("BTC-USD", expiries=[_D2], spot_pct_range=None)
...
def test_put_premium_apr_and_iv(fake_sdk):
    """80000-P ask=0.0046（面值比例 0.46%），lot=0.01 BTC，spot=81000。

    prem_usd = 0.0046 × 0.01 × 81000 = 3.726；apr = 0.46% × 365 / 1 天
    """
    chain = od.fetch_chain("BTC-USD", expiries=[_D1], spot_pct_range=None)
    ...
    assert row["C"]["bid"] == 0.05


def test_hv_from_closes(fake_sdk):
    hv = od._spot_hv("BTC-USD", 30)
    assert hv["days"] == 4
