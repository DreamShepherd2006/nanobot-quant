...
RUN echo "[bust=40]" && pip install --break-system-packages \
        git+https://github.com/DreamShepherd2006/nanobot-legion.git@6f946c7f \
    && python3 -m nanobot_legion.install \
    && echo "✅ nanobot-legion @6f946c7f (upstream staging — PR #49 回测页路由+业务入口, bust=40)"
...
RUN echo "[bust=189]" && pip install --break-system-packages \
        git+https://github.com/DreamShepherd2006/nanobot-quant.git@cff27af \
        git+https://github.com/DreamShepherd2006/Vibe-Trading.git@v0.1.12 \
    && echo "✅ nanobot-quant @cff27af (upstream main — PR #235 高9 盈利门 sell_only_profit + td_sell_all 全平 + gate_symbol 键映射修复, bust=189) + vibe-trading @v0.1.12"
