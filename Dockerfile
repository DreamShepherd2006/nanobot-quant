...
RUN echo "[bust=630]" && pip install --break-system-packages \
        'mcp<2' \
        git+https://github.com/DreamShepherd2006/nanobot-quant.git@0e760b1 \
        git+https://github.com/DreamShepherd2006/Vibe-Trading.git@v0.1.12 \
    && echo "✅ nanobot-quant @0e760b1 (F1 模式 S1+S2+S3+S5 + analyze_f1_td 口径修正（改按价格度量）+ 大周期趋势列名/错误可见; bust=630) + vibe-trading @v0.1.12"
